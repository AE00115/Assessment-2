...
float celsius;  
  ...
  while (1) //Never Ending while loop (so it can keep on working all the time)
  {
    ...
    //The string is placed in the buffer passed, which must be large enough to hold the output.
     ...
     itoa(counter2,dwn,10);// conversion to understand the value
  ...
  // SELECT = <800
    ...
    if(x<60) {  //RIGHT
    // if right button is pressed increase the value of counter2 (lower limit) by 2 
    counter2 = counter2 +2;
    LCD_Clear(); // clear the screen

  }
    else if( x<200) {  //UP
    // if up button is pressed increase the value of counter (upper limit) by 2 
     counter = counter +2;
     LCD_Clear(); // clear the screen 
    }  
    else if( x<400) { //DOWN
    // if up button is pressed decrease the value of counter (upper limit) by 2 
    counter = counter -2 ;
    LCD_Clear(); // clear the screen
      }

    else if(x<600) { //LEFT
    // if leftbutton is pressed decrease the value of counter2 (lower limit) by 2 
    counter2 = counter2 - 2 ;
    LCD_Clear(); // clear the screen
     }
    ...
    LCD_Clear(); // clear the screen
    LCD_String_xy(0,0,"Lower: ");  // 1. print on the LCD "Lower: "
    LCD_String_xy(0,6,dwn);        // 1. print on the LCD the Value of the lower limit
    LCD_String_xy(1,0,"Upper:");   // 1. print on the LCD "Upper:"
    ...
    counter2 = counter2;
      ...
      if(counter<t){  // if t (temperature) exceeded the upper limit
        PORTB = PORTB | (1 <<5); // RED LED ON 
      }

      else if(t<counter2 ){  // if t (temperature) is below the lower limit

      PORTB = PORTB | (1 << 4); //GREEN LED ON
       }


      else if(t>counter2 ){ // if t (temperature) exceeded the lower limit
        PORTB = PORTB  &!(1 <<4); //GREEN LED OFF
...
void init()
{
  ...
  DDRC &= ~(1 << 2);   // Make it input


  DDRB = DDRB | (1 << 4); // make PB4 output
      LCD_Init ();
      ADC_Init();
}